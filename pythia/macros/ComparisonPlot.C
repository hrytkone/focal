	...
	for (int iset = 0; iset < nset; iset++) {
		for (int itrigg = 0; itrigg < nTriggBins; itrigg++) {
			...
			for (int iassoc = 0; iassoc < nAssocBins; iassoc++) {
				...
				hCorrMassMass[iset][itrigg][iassoc] = (TH1D*)fin[iset]->Get(Form("hCorrMassMass[%4.1f,%4.1f][%4.1f,%4.1f]_px",tlow,tupp,alow,aupp)); hCorrMassMass[iset][itrigg][iassoc]->Rebin(2);
    ...
    for (int iset = 0; iset < nset; iset++) {
        for (int itrigg = 0; itrigg < nTriggBins; itrigg++) {
            ...
            for (int iassoc = 0; iassoc < nAssocBins; iassoc++) {
                ...
                hCorrMassMass[iset][itrigg][iassoc]->SetMarkerSize(0.5);
